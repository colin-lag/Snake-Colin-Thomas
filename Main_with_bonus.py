...
size_barre = 70
vitesse = 75.0
duree_effet = duree_bombe = 0
inverse = bombe_obj = False
...
bonus = pygame.image.load("bonus.png").convert_alpha() #Le fruit bonus
bonus = pygame.transform.scale(bonus, (35,35))
bombe = pygame.image.load("bombe.png").convert_alpha() # La bombe
bombe = pygame.transform.scale(bombe, (35,35))

#Récuperation de leur position
position_1 = head.get_rect()
position_fruit = fruit.get_rect()
position_bonus = bonus.get_rect()
position_bombe = bombe.get_rect()
...
#Position du fruit bonus en dehors de l'écran
position_bonus.x = -50
position_bonus.y = -50

#Position de la bombe en dehors de l'écran
position_bombe.x = -50
position_bombe.y = -50

apparition_bonus = randint(5,15)
apparition_bombe = randint(5,25)
...
while(continuer):
        ...
        if event.type == pygame.KEYDOWN:#Vérification de si le joueur appuye sur une des touches du clavier
            ...
            if event.key == pygame.K_UP:
                if etat == 2: #Vérification de si le programme est à l'état de jeu
                    if depUp == False and move_init == True:#Vérification que la direction soit différente et annonce que les déplacement on débutés
                        ...
                        if inverse:
                            depUp = depRight = depLeft = False #Changement de la variable de déplacement
                ...
                if etat == 2:
                    if depDown == False:# Empêchement d'aller dans la direction opposée
                        ...
                        if inverse == False:
                            ...
                            pygame.mixer.Sound.play(bruit_mouvement)
                        if inverse:
                            depRight = depLeft = depDown = False #Changement de la variable de déplacement
                            depUp = move_init = True
                            pygame.mixer.Sound.play(bruit_mouvement)
                ...
                if etat == 2:
                    if depRight == False: # Empêchement d'aller dans la direction opposée
                        ...
                        if inverse == False:
                            ...
                            pygame.mixer.Sound.play(bruit_mouvement)
                        if inverse:
                            depRight = depUp = depDown = False #Changement de la variable de déplacement
                ...
                if etat == 2:
                    if depLeft == False:
                        ...
                        if inverse == False:
                            ...
                            pygame.mixer.Sound.play(bruit_mouvement)
                        if inverse:
                            depLeft = depDown = depUp = False #Changement de la variable de déplacement
                            depRight = move_init = True
                            pygame.mixer.Sound.play(bruit_mouvement)
            ...
            if event.key == pygame.K_RETURN:
                ...
                if etat == 3:
                    ...
                    apparition_bonus = randint(5,15)
                    apparition_bombe = randint(5,25)
                    position_bonus.x = position_bonus.y = position_bombe.x =  position_bombe.y = -50
                    inverse = False
                    duree_bombe = duree_effet = 0
    ...
    if etat == 2:
        ...
        fenetre.blit(bonus, position_bonus)
        fenetre.blit(bombe, position_bombe)
        ...
        if score == apparition_bonus:
            position_bonus.x = randint(1,20)*step   #Nouvelles coordonnées du bonus
            position_bonus.y = randint(1,20)*step
            apparition_bonus = randint(score, score + 15)

        if collision(x[0], y[0], position_bonus.x, position_bonus.y,35,25):
            position_bonus.x = -50  #Nouvelles coordonnées du bonus lorsqu'il est "mangé"
            position_bonus.y = -50
            length = length + 5
            score = score + 5
        ...
        #Collision avec fruit mystere inverse les controles pendant un nombre de mouvement donnée

        if score >= apparition_bombe:
            position_bombe.x = randint(1,20)*step   #Nouvelles coordonnées du bonus
            position_bombe.y = randint(1,20)*step
            apparition_bombe = randint(score, score + 25)
            bombe_obj = True

        if collision(x[0], y[0], position_bombe.x, position_bombe.y,35,25):
            position_bombe.x = -50
            position_bombe.y = -50
            inverse = True

        if bombe_obj:
            duree_bombe = duree_bombe + 1

        if duree_bombe == 100:
            position_bombe.x = position_bombe.y = -50

        if inverse:
            duree_effet = duree_effet + 1
            if duree_effet > 100:
                inverse = False
